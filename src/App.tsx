...
function CreatePositionModal({ type, artifacts: initialArtifacts, initialArtifact, onClose }: { type: 'raffle' | 'auction' | 'loan' | 'choose'; artifacts: OwnedArtifact[]; initialArtifact?: OwnedArtifact; onClose: () => void }) {
  const { address } = useAccount()
  const { data: artifacts = initialArtifacts, isPending: nftsLoading, isError: nftsError, isFetching: nftsFetching, refetch: retryNfts } = useOwnedNfts(address)
  const { runAction } = useActions()
  const titleId = useId()
  const [selectedType, setSelectedType] = useState(type === 'choose' ? 'raffle' : type)
  const [selectedArtifactKey, setArtifactKey] = useState(initialArtifact ? `${initialArtifact.collection}:${initialArtifact.tokenId}` : '')
  const artifactKey = artifacts.some(item => `${item.collection}:${item.tokenId}` === selectedArtifactKey) ? selectedArtifactKey : artifacts[0] ? `${artifacts[0].collection}:${artifacts[0].tokenId}` : ''
  ...
  return (
    <div className="modal-backdrop" onPointerDown={event => event.target === event.currentTarget && requestClose()}>
      <section ref={dialogRef} className="wallet-modal create-modal" role="dialog" aria-modal="true" aria-labelledby={titleId} tabIndex={-1}>
        ...
        {nftsLoading ? <div role="status"><EmptyState title="Loading your NFTs…" copy="Checking the NFTs available in your wallet." /></div> : nftsError ? <div className="empty-tab" role="alert"><h3>Couldn't load your NFTs</h3><p>We couldn't verify your complete NFT list. Please retry.</p><button type="button" className="button button--ghost" disabled={nftsFetching} onClick={() => { void retryNfts() }}>{nftsFetching ? 'Retrying…' : 'Retry'}</button></div> : artifacts.length === 0 ? <div><EmptyState title="No available NFTs" copy="No NFTs from supported collections are currently available in this wallet. Check Vault for escrowed positions and any return or claim actions." /><button type="button" className="button button--ghost" disabled={nftsFetching} onClick={() => { void retryNfts() }}>{nftsFetching ? 'Checking…' : 'Refresh NFTs'}</button></div> : <div className="create-form">
      ...
      <div className="detail-grid">
        ...
        <section className="detail-copy">
          ...
          <div className="action-panel">
            ...
            {raffle.state === 1 && isCreator && !raffle.proceedsClaimed && <button className="button button--ghost button--wide" onClick={() => runAction('Claiming ticket fee', `Raffle #${raffle.id} ticket fee claimed`, { kind: 'raffleLifecycle', id: raffle.id, functionName: 'claimProceeds' })}>Claim ticket fee</button>}
      ...
      <div className="detail-grid">
        ...
        <section className="detail-copy">
          ...
          <div className="action-panel">
            ...
            {auction.state === 2 && ((auction.sold && isWinner) || (!auction.sold && isSeller)) && !auction.nftClaimed && <button className="button button--primary button--wide" onClick={() => runAction('Claiming NFT', `NFT #${auction.tokenId} claimed`, { kind: 'auctionLifecycle', id: auction.id, functionName: 'claimNFT' })}>{auction.sold ? 'Claim NFT · winner only' : 'Claim NFT · seller only'}</button>}
            {auction.state === 2 && auction.sold && isSeller && !auction.proceedsClaimed && <button className="button button--ghost button--wide" onClick={() => runAction('Claiming proceeds', `Auction #${auction.id} proceeds claimed`, { kind: 'auctionLifecycle', id: auction.id, functionName: 'claimProceeds' })}>Claim proceeds · seller only</button>}
            {auction.state === 2 && auction.sold && !isSeller && !isWinner && <p className="action-note"><ShieldCheck size={14} />Only the winning bidder can claim the NFT, and only the seller can claim the auction proceeds.</p>}
  ...
  return (
    <main>
      ...
      <section className="guide wrap">
        <aside className="guide__aside"><span>IN THIS GUIDE</span><a href="#start">01 · Arc Testnet</a><a href="#rituals">02 · Three utilities</a><a href="#safety">03 · Staying safe</a><a href="#planning">04 · Future planning</a><a href="#questions">05 · Questions</a></aside>
        <div className="guide__content">
          ...
          <article id="planning" className="guide-section"><span className="guide-number">04</span><div><span className="eyebrow">Future planning</span><h2>Designed to open up over time.</h2><div className="ritual-list"><div><Landmark /><span><b>Curated lending</b><p>Only selected NFT collections will be eligible for lending, so collateral can follow clear risk and allowlist rules.</p></span></div><div><Gavel /><span><b>Open auctions and raffles</b><p>Any supported NFT collection—or an independent NFT—can create an auction or raffle.</p></span></div><div><Sparkles /><span><b>Testnet collections</b><p>Ursa Arcana collections are built for Arc Testnet experimentation and testing only, not as a promise of mainnet value.</p></span></div></div></div></article>
          <article id="questions" className="guide-section"><span className="guide-number">05</span><div><span className="eyebrow">Questions</span><h2>The details, made simple.</h2><div className="faq">{faqs.map(([question, answer], index) => { const panelId = `faq-panel-${index}`; const buttonId = `faq-button-${index}`; const isOpen = open === index; return <div className={`faq-item ${isOpen ? 'open' : ''}`} key={question}><button type="button" id={buttonId} aria-expanded={isOpen} aria-controls={panelId} onClick={() => setOpen(isOpen ? -1 : index)}><span>{question}</span><Plus size={18} aria-hidden="true" /></button><div id={panelId} role="region" aria-labelledby={buttonId} aria-hidden={!isOpen}><p>{answer}</p></div></div> })}</div></div></article>
        </div>
      </section>
    </main>
  )
...
function Footer() {
  return (
    <footer className="site-footer">
      <div className="wrap footer-main"><div className="footer-brand"><Link className="brand" to="/"><UrsaMark compact /><span><b>Ursa</b><em>Arcana</em></span></Link><p>One home for collecting, exchanging, and unlocking NFT utility on Arc Testnet.</p></div><div className="footer-col"><b>Explore</b><Link to="/raffles">Raffles</Link><Link to="/auctions">Auctions</Link><Link to="/lend">Lending</Link><Link to="/vault">Vault</Link></div><div className="footer-col"><b>Resources</b><Link to="/learn">How it works</Link><a href="https://docs.arc.io" target="_blank" rel="noreferrer">Arc docs</a><a href="https://testnet.arcscan.app" target="_blank" rel="noreferrer">ArcScan</a><a className="footer-external-link" href="https://x.com/ursaarcana" target="_blank" rel="noreferrer" aria-label="Follow Ursa Arcana on X, opens in a new tab">X · @ursaarcana <ArrowUpRight aria-hidden="true" /></a></div><div className="footer-status"><span className="eyebrow">Network status</span><b><i />Arc Testnet live</b><small>Chain 5,042,002 · Testnet USDC</small></div></div>
      <div className="wrap footer-bottom"><span>© 2026 Ursa Arcana</span><span>Testnet assets have no monetary value.</span><a className="footer-credit" href="https://github.com/rrreol69" target="_blank" rel="noreferrer" aria-label="Built by rrreol69, opens GitHub in a new tab">Built by rrreol69 <ArrowUpRight aria-hidden="true" /></a></div>
    </footer>
  )
}
  ...
  const runAction = async (label: string, success: string, transaction?: ContractAction) => {
    ...
    try {
      ...
      if (transaction.kind === 'mint' || transaction.kind === 'mintCollection') {
        ...
        const currentOwner = await publicClient.readContract({ address: transaction.nftContract, abi: nftAbi, functionName: 'ownerOf', args: [BigInt(transaction.tokenId)] })
        if (!sameAddress(currentOwner, address)) throw new Error('This NFT is no longer available in your wallet. Refresh your NFTs and try again.')
        const allowed = await publicClient.readContract({ address: spender, abi: nftAbi, functionName: 'allowedCollections', args: [transaction.nftContract] })
        if (!allowed) throw new Error('This collection is not supported by the selected utility.')
